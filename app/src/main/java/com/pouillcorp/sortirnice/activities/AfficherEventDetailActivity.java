...
public class AfficherEventDetailActivity extends NavDrawerActivity {
    ...
    private void hideFields() {
        if (!isSauvegarde) {
            ...
            if (eventTransmis.getAdressSituationContent() == null) {
                adressContent.setVisibility(View.GONE);
            }
            if (eventTransmis.getAdressSituationZip() == null) {
                adressZip.setVisibility(View.GONE);
            }
            if (eventTransmis.getAdressSituationCity() == null) {
    ...
    private void fillAllFields() {
        if (!isSauvegarde) {
            ...
            adressContent.setText(eventTransmis.getAdressSituationContent());
            adressZip.setText(eventTransmis.getAdressSituationZip());
            adressCity.setText(eventTransmis.getAdressSituationCity());
    ...
    public void save(View view) {
        ...
        current.setAdressContent(eventTransmis.getAdressSituationContent() != null ? eventTransmis.getAdressSituationContent() : null);
        current.setAdressZip(eventTransmis.getAdressSituationZip() != null ? eventTransmis.getAdressSituationZip() : null);
        current.setAdressCity(eventTransmis.getAdressSituationCity() != null ? eventTransmis.getAdressSituationCity() : null);
    ...
    public void launchGoogleMap(View view) {
        ...
        } else if (eventTransmis.getAdressSituationContent() != null || eventTransmis.getAdressSituationZip() != null
        || eventTransmis.getAdressSituationCity() != null) {
            url += "0,0?q=";
            addr += Uri.parse(eventTransmis.getAdressSituationContent()+" "+eventTransmis.getAdressSituationZip()+" "+eventTransmis.getAdressSituationCity());
            url += addr;
        }
    ...
    public void launchWaze(View view) {
        try
        {
            ...
            if (eventTransmis.getLatitude() !=0d && eventTransmis.getLongitude()!=0d) {
                ...
                url += eventTransmis.getAdressSituationContent()+" " +eventTransmis.getAdressSituationZip()+" " + eventTransmis.getAdressSituationCity();
            }
            Intent intent = new Intent( Intent.ACTION_VIEW, Uri.parse( url ) );
            startActivity( intent );
        }
